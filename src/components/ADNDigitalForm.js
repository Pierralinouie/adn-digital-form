...
const ADNDigitalForm = () => {
  ...
  const calculateIdentityScore = (data) => {
    let score = 0;
    if (data.styleMusical) score += 15;
    if (data.universVisuel) score += 15;
    if (data.histoire) score += 10;
    if (data.influences) score += 10;
    if (data.couleursSignature) score += 10;
    return score;
  };

  const generateRecommendations = (data) => {
    const recs = [];
    if (!data.styleMusical) recs.push("Définir votre style musical en 3 mots-clés");
    if (!data.universVisuel) recs.push("Développer une identité visuelle cohérente");
    if (!data.histoire || data.histoire.length < 100) {
      recs.push("Enrichir votre storytelling avec plus de détails personnels");
    }
    return recs;
  };
    ...
    pillars: [
      {
        ...
        formats: ["Teasers", "Live sessions", "Clips", "Covers"]
      },
      {
        type: "Contenu Personnel",
        frequency: "30% du contenu total",
        formats: ["Behind-the-scenes", "Daily stories", "Q&A"]
      },
      {
        type: "Contenu Éducatif",
        frequency: "15% du contenu total",
        formats: ["Tutoriels", "Processus créatif", "Conseils"]
      },
      {
        type: "Contenu Engageant",
        frequency: "15% du contenu total",
        formats: ["Sondages", "Challenges", "UGC"]
      }
    ]
  });
